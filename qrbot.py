...
def check_daily_limit(user):
    ...
    if user.get("last_reset") != today:
        users_col.update_one({"user_id": user["user_id"]}, {"$set": {"daily_count": 0, "last_reset": today}})
        user["daily_count"] = 0
...
def log_qr_generation(user_id, amount):
    logs_col.insert_one({"user_id": user_id, "amount": amount, "time": datetime.datetime.utcnow()})
...
def qr_handler(message):
    ...
    if check_cooldown(user):
        bot.reply_to(message, "⏳ Cooldown active. Please wait before generating another QR.")
        return

    if check_daily_limit(user):
        bot.reply_to(message, "⚠️ Daily limit reached. Upgrade to premium for unlimited QR generation.")
        return
    ...
    users_col.update_one(
        ...
        {"$inc": {"daily_count": 1}, "$set": {"last_qr_time": datetime.datetime.utcnow()}}
    )
